...
class SimpleShot(nn.Module):
    ...
    @torch.no_grad()
    def nearest_neighbor_classification(
        self, query_images, support_images, support_labels
    ):
        ...
        if self.l2norm:
            query_features = query_features / (
                1e-8
                + torch.linalg.vector_norm(query_features, ord=2, dim=-1, keepdim=True)
            )
            support_features = support_features / (
                1e-8
                + torch.linalg.vector_norm(
                    support_features, ord=2, dim=-1, keepdim=True
                )
            )
